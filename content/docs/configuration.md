---
{"publish":true,"title":"Configuration","created":"2025-08-14T17:29:12.145+02:00","modified":"2025-08-14T17:29:12.145+02:00","cssclasses":""}
---


Quartz is meant to be extremely configurable, even if you don't know any coding. Most of the configuration you should need can be done by just editing `quartz.config.ts` or changing [[docs/layout\|the layout]] in `quartz.layout.ts`.
...
- `pageTitle`: title of the site. This is also used when generating the [[docs/features/RSS Feed]] for your site.
- `pageTitleSuffix`: a string added to the end of the page title. This only applies to the browser tab title, not the title shown at the top of the page.
- `enableSPA`: whether to enable [[docs/features/SPA Routing]] on your site.
- `enablePopovers`: whether to enable [[docs/features/popover previews]] on your site.
...
- `locale`: used for [[docs/features/i18n]] and date formatting
- `baseUrl`: this is used for sitemaps and RSS feeds that require an absolute URL to know where the canonical 'home' of your site lives. This is normally the deployed URL of your site (e.g. `quartz.jzhao.xyz` for this site). Do not include the protocol (i.e. `https://`) or any leading or trailing slashes.
  - This should also include the subpath if you are [[docs/hosting]] on GitHub pages without a custom domain. For example, if my repository is `jackyzha0/quartz`, GitHub pages would deploy to `https://jackyzha0.github.io/quartz` and the `baseUrl` would be `jackyzha0.github.io/quartz`.
  - Note that Quartz 4 will avoid using this as much as possible and use relative URLs whenever it can to make sure your site works no matter _where_ you end up actually deploying it.
- `ignorePatterns`: a list of [glob](<https://en.wikipedia.org/wiki/Glob_(programming)>) patterns that Quartz should ignore and not search through when looking for files inside the `content` folder. See [[docs/features/private pages]] for more details.
...
- `theme`: configure how the site looks.
  ...
  - `colors`: controls the theming of the site.
    ...
    - `secondary`: link colour, current [[docs/features/graph view\|graph]] node
    - `tertiary`: hover states and visited [[docs/features/graph view\|graph]] nodes
    - `highlight`: internal link background, highlighted text, [[docs/features/syntax highlighting\|highlighted lines of code]]
...
You should take care to add the plugin to the right entry corresponding to its plugin type. For example, to add the [[docs/plugins/ExplicitPublish]] plugin (a [[tags/plugin/filter\|Filter]]), you would add the following line:
...
If you'd like to make your own plugins, see the [[docs/advanced/making plugins\|making custom plugins]] guide.
